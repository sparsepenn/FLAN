...
class DANN(object):
    ...
    # def _generate_batch(self, image, label, min_queue_examples,
    #                     batch_size, shuffle):
    ...
    #
    # def read_mnistm(self, filename, is_train):
    #     if is_train:
    ...
    #     return img, label
    #
    # def load_batch_mnistm(self, batch_size, data_dir, dataname, is_train=False, shuffle=True):
    #     filename = os.path.join(data_dir, dataname)
    #     image, label = self.read_mnistm(filename, is_train)
    #     min_queue_examples = 5000
    #     images, labels = self._generate_batch(image, label,
    ...
    # def read_mnist(self, filename, is_rgb, is_train, size=28):
    #     if is_train:
    #         filename_queue = tf.train.string_input_producer([filename])
    #     else:
    #         filename_queue = tf.train.string_input_producer([filename], num_epochs=1)
    ...
    #     return img, label
    #
    # def load_batch_mnist(self, batch_size, data_dir, dataname,
    #                      is_train=False, is_rgb=False,
    #                      size=28, shuffle=True):
    #     filename = os.path.join(data_dir, dataname)
    #     image, label = self.read_mnist(filename, is_rgb, is_train, size)
    #     min_queue_examples = 5000
    #     images, labels = self._generate_batch(image, label,
    #                                           min_queue_examples, batch_size,
    #                                           shuffle=shuffle)
    #     # images = tf.reshape(images, [-1, size, size, 1])
                ...
                with slim.arg_scope([slim.conv2d], kernel_size=[5, 5], stride=1, padding="SAME"):
                    with slim.arg_scope([slim.max_pool2d], kernel_size=[2, 2], stride=2, padding="SAME"):
                        with slim.arg_scope([slim.batch_norm], decay=0.80, center=True, scale=True, epsilon=1e-5,
                            ...
                            # # [28,28,1]---[14,14,64]
                            # net = slim.conv2d(net, 32, scope='conv0')
                            ...
                            # [14,14,32]---[7,7,48]
                            net = slim.conv2d(net, 64, scope='conv1')
                            net_show = slim.max_pool2d(net, scope='pool1')
                            net = slim.batch_norm(net_show, scope='bn1')
                            # [7,7,128] ---[7,7,128]
                            net = slim.conv2d(net, 48, scope='conv2')
                            net = slim.batch_norm(net, scope='bn2')

                            # reshape[7,7,128]---[7*7*128](6077)
                            net = tf.reshape(net, [-1, 7 * 7 * 48])
                            # [7*7*128](6077)---[500](500 - 200)
                            net = slim.fully_connected(net, 800, scope="fc3")
                            net = slim.batch_norm(net, scope='bn3')
                            # [100]---[10]no activation function
                            net = slim.fully_connected(net, 10, scope="fc4")
                            # return tf.reshape(net_show, [-1, 7 * 7 * 48])
                            return net
    ...
    def build_model(self, source_images, target_images, source_labels, target_labels, step, is_train=True):
        ...
        g_train_op = tf.train.MomentumOptimizer(lr, 0.9).minimize(g_loss, None, g_vars)
        c_train_op = tf.train.MomentumOptimizer(lr, 0.9).minimize(c_loss, None, c_vars)
    ...
    def test_model(self, step):
        with tf.Graph().as_default():
            global_step = tf.Variable(trainable=False, initial_value=0, dtype=tf.int32)
            test_target_images, test_target_labels = ops.load_batch_mnistm(1000,
